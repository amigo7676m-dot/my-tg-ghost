...
void ApiWrap::markContentsRead(
	...
	for (const auto& item : items) {
		item->markContentsRead(true); // Помечаем как прослушанное локально для тебя
	}
	return; // <-- Блокируем отправку MTPmessages_ReadMessageContents и MTPchannels_ReadMessageContents в сеть
}

void ApiWrap::markContentsRead(not_null<HistoryItem*> item) {
	item->markContentsRead(true); // Помечаем как прослушанное локально для тебя
	return; // <-- Блокируем отправку в сеть
}
